    ...
    <flux:sidebar sticky collapsible="mobile"
        ...
        <flux:sidebar.nav class="flex-1 px-2">
            ...
            <div class="mb-4">
                ...
                <flux:sidebar.item icon="squares-plus" :href="route('admin.services')"
                    :current="request()->routeIs('admin.services')" wire:navigate>
                    {{ __('Services') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="user-circle" :href="route('admin.staff')"
                    :current="request()->routeIs('admin.staff')" wire:navigate>
                    {{ __('Staff') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="users" :href="route('admin.customers')"
            ...
            <div class="mb-4">
                ...
                <flux:sidebar.item icon="shield-check" :href="route('admin.admins')"
                    :current="request()->routeIs('admin.admins')" wire:navigate>
                    {{ __('Admins') }}
                </flux:sidebar.item>
                <flux:sidebar.item icon="key" :href="route('admin.roles')" :current="request()->routeIs('admin.roles')"
                    wire:navigate>
                    {{ __('Roles') }}
                </flux:sidebar.item>
            </div>
        ...
        <div class="border-t border-zinc-100 dark:border-zinc-800 p-2">
            <flux:dropdown position="top" align="start" class="w-full">
                <button
                    ...
                    <div
                        class="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-fuchsia-500 to-fuchsia-600 text-sm font-semibold text-white">
                        {{ auth()->user()->initials() }}
                    </div>
    ...
    <flux:header class="lg:hidden">
        ...
        <flux:dropdown position="top" align="end">
            <button class="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-zinc-100 dark:hover:bg-zinc-800">
                <div
                    class="flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-to-br from-fuchsia-500 to-fuchsia-600 text-sm font-semibold text-white">
                    {{ auth()->user()->initials() }}
                </div>
